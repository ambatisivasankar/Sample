...
for _ in {1..121}
  ...
  ./load_tables.bash "${ENV}" --sql-file=squark-classic/job-sql/${squark_name}/truncate_AGMT_FIN_TXN_CMN_VW_qa.sql
  ...
  if [ -z "${strt_dt}" ]
    then
      ...
      --sql-file=squark-classic/job-sql/${squark_name}/insert_IDL_batch_AGMT_FIN_TXN_CMN_VW_qa.sql./load_tables.bash "${ENV}"
      --sql-file=squark-classic/job-sql/${squark_name}/update_batch_dates_AGMT_FIN_TXN_CMN_VW_qa.sql
  fi
