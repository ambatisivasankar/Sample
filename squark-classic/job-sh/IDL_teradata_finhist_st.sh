...
for ((i=0;i<=NUM_LOOPS; i++));
  ...
  if [ -z "${strt_dt}" ]
    then
      ...
      ./load_tables.bash "${ENV}" --sql-file=squark-classic/job-sql/${squark_name}/insert_IDL_batch_AGMT_FIN_TXN_CMN_VW_st.sql
      ./load_tables.bash "${ENV}" --sql-file=squark-classic/job-sql/${squark_name}/update_batch_dates_AGMT_FIN_TXN_CMN_VW_st.sql
  fi
