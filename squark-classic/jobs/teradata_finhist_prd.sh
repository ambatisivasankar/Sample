# Required
export PROJECT_ID=teradata_finhist_prd
...
if [ "${IDL}" -eq 1 ]; then
  ...
  export JSON_INFO="
  {
      'SAVE_TABLE_SQL_SUBQUERY':{
          'AGMT_FIN_TXN_CMN_VW': {
              'sql_query': '(SELECT * FROM PROD_USIG_STND_VW.AGMT_FIN_TXN_CMN_VW where TRANS_EFFECTIVE_DATE  between cast('''$strt_dt''' as date) AND cast('''$end_dt''' as date)) as subquery',
              'numPartitions': 10,
      ...
      'TABLE_MAP':{
          'AGMT_FIN_TXN_CMN_VW': 'AGMT_FIN_TXN_CMN_VW_prd'
      }
  }
  "
else
  ...
  export JSON_INFO="
  {
      'SAVE_TABLE_SQL_SUBQUERY':{
          'AGMT_FIN_TXN_CMN_VW': {
              'sql_query': '(SELECT * FROM PROD_USIG_STND_VW.AGMT_FIN_TXN_CMN_VW where TRANS_DT  between cast('''$end_dt''' as date) - $DELTA_RANGE AND cast('''$end_dt''' as date)) as subquery',
              'numPartitions': $PARTS,
